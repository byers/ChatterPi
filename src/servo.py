#
# Servo and Angular Servo wrappers for Adafruit ServoKit and Pololu: based on a library for controlling the Raspberry Pi's GPIO pins
#
...
class Servo:
    """
    Use Adafruit ServoKit or Pololu controller for driving 6-16 servo control board.
    ...
    def value(self, value):
        ...
        if value is None:
            self._value = None
            self._servo.set_target(0, 0) 
        elif -1 <= value <= 1:
            self._value = value

            fraction = round((value - self._min_value) / self._value_range, 5)
            print("Value: %f" % (fraction))
            pulse_width = (fraction * (self.max_pulse_width - self.min_pulse_width) + self.min_pulse_width)
            print("Pulse Width: %f" % (pulse_width))

            # Send fraction (-1 to 1) and pulse width to servo adapter
            self._servo.set_target(fraction, pulse_width) 
